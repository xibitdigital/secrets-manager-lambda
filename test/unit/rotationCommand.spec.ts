import {
	...
	rotateSecret,
	makeToken,
} from "../../src/rotateCommand";
import { SecretsManagerRotateCommand } from "../../src/types";

const sampleSecret = { foo: "FrfiwOxkCT", bar: "GL30HeLOOi" };

const commandSample: SecretsManagerRotateCommand = {
	action: "rotate",
	secretArn: "foo",
	keys: ["bar"],
};
...
describe("rotateCommand", () => {
	...
	describe("rotateSecret", () => {
		it("should update only the secret that has bar as key", () => {
			...
			const res: any = rotateSecret(sampleSecret, ["bar"]);
